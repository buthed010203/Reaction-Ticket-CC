...
{{if and (eq $RM.Author.ID 204255221017214977) (reFind `These are your server tickets:` $RM.Content)}}
    {{$checkPage := reFind `Page: [0-9]+` .ReactionMessage.Content}}{{$page = reFind `[0-9]+` $checkPage}}
    ...
    {{if ge (toInt $page) 1}}
        ...
        {{if len $tickets}}
            ...
            {{end}}
        {{$embed = (joinStr "" "These are your server tickets:\nPage: " $page "\n```" $msg "```")}}
...
{{end}}
{{if $isTicketEmbed}}
    {{deleteMessageReaction nil $RM.ID $user $upEmoji $downEmoji}}
    {{if eq $emoji $upEmoji}}{{$page = add $page 1}}{{else if eq $emoji $downEmoji}}{{$page = sub $page 1}}{{end}}
    {{if ge (toInt $page) 1}}
        ...
        {{if len $tickets}}
            ...
            {{$embed = cembed "title" $titulo "fields" $fields "color" 3066993 "footer" (sdict "text" (joinStr "" "Page: " $page))}}
