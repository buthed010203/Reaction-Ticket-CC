...
{{if not (eq (toInt $master.pos) 3)}}
    {{if (eq $alert 1)}}
        {{$master.Set "duration" (currentTime.Add (toDuration "2h30m"))}}
        ...
        {{$content := joinStr "" "O ticket " $tn " será fechado em breve!\n\n" mentionEveryone}}
    ...
    {{else if eq $alert 2}}
        ...
        {{$content := joinStr "" "O ticket " $tn " está prestes a ser fechado!\n\n" mentionEveryone}}
        {{sendMessageNoEscape nil (complexMessage "content" $content "embed" $embed)}}
